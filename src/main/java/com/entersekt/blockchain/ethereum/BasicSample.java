...
/**
 * The base sample class which creates EthereumJ instance, tracks and report all the stages of starting up like
 * discovering nodes, connecting, syncing
 * 
 * The class can be started as a standalone sample it should just run until full blockchain sync and then just hanging,
 * listening for new blocks and importing them into a DB
 * 
 * This class is a Spring Component which makes it convenient to easily get access (autowire) to all components created
 * within EthereumJ. However almost all this could be done without dealing with the Spring machinery from within a
 * simple main method
 * 
 * Created by Anton Nashatyrev on 05.02.2016.
 * 
 * koosiedemoer: took out all the Spring & config stuff
 */
public class BasicSample implements Runnable {
	...
	private String loggerName = "Sample";
	...
	public BasicSample() {
		...
		adminInfo = ethereum.getAdminInfo();
	...
	/**
	 * logger name can be passed if more than one EthereumJ instance is created in a single JVM to distinguish logging
	 * output from different instances
	 */
	...
	/**
	 * Allow only selected logger to print DEBUG events to STDOUT and FILE. Other loggers are allowed to print ERRORS
	 * only.
	 */
	...
	/**
	 * The method tracks step-by-step the instance lifecycle from node discovery till sync completion. At the end the
	 * method onSyncDone() is called which might be overridden by a sample subclass to start making other things with
	...
	/**
	 * Discovering nodes is only the first step. No we need to find among discovered nodes those ones which are live,
	 * accepting inbound connections, and has compatible subprotocol versions
	 */
	...
	/**
	 * When live nodes found SyncManager should select from them the most suitable and add them as peers for syncing the
	 * blocks
	 */
